...
dat$rsFirst <- gsub("^(\\w{1,4}).*", "\\1", dat$avsnp150)

prevarList.rsID <- dat[, c("HG19_ID", "Chr", "Start", "End", "Gene.refGene", "avsnp150", "rsFirst")]

colnames(prevarList.rsID) <- c("id", "chr", "37bp1", "37bp2", "geneID", "rsID", "rsFirst")
for (i in 1:nrow(prevarList.rsID)) {
      prevarList.rsID$id[i] <- paste0('<a id="', prevarList.rsID$id[i], '" href="javascript:;"onclick="resClick(this.id)">', prevarList.rsID$id[i], '</a>')
}
varList.rsID.nested <- split(prevarList.rsID, dat$rsFirst)
varList.rsID.nested[1] <- NULL
for (i in 1:length(varList.rsID.nested)) {
  varList.rsID.nested[[i]]$rsFirst <- NULL
}

save(varList.rsID.nested, file = "../www/rsID.List.RData")
```
